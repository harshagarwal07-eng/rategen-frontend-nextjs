...
interface HotelPricingTabProps {
...
export function HotelPricingTab({ ctx }: HotelPricingTabProps) {
  const {
    formData,
    hotelDetails,
    breakups,
  ...
  // Calculate max days - use itinerary or derive from breakups/formData
  const maxDays = itineraryInfo?.nights
    ? itineraryInfo.nights + 1
    : Math.max(formData.day_number || 7, ...breakups.map((b) => b.day_number || 0), 7);

  // Generate available days based on itinerary
  const availableDays = Array.from({ length: maxDays }, (_, i) => i + 1);

  // Get check-in date from itinerary or formData
  const checkInDate = itineraryInfo?.checkIn || formData.check_in_date || formData.day_date;
  ...
  // Formula: (Base * Qty) - Discount + Markup + Tax
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex-1 space-y-4 p-3">
        ...
        {(hotelDetails?.other_details || hotelDetails?.extra_bed_policy || hotelDetails?.stop_sale) && (
          <section className="space-y-2">
            <h3 className="text-xs font-medium text-muted-foreground">Room Policies</h3>
            <div className="grid grid-cols-3 gap-3 text-xs">
              <div className="p-2 bg-muted/30 rounded">
                <div className="text-[10px] text-muted-foreground mb-1">Other Details</div>
                <RategenMarkdown content={hotelDetails.other_details || "N/A"} className="text-xs" />
              </div>
              <div className="p-2 bg-muted/30 rounded">
                <div className="text-[10px] text-muted-foreground mb-1">Extra Bed Policy</div>
                <RategenMarkdown content={hotelDetails.extra_bed_policy || "N/A"} className="text-xs" />
              </div>
              <div className="p-2 bg-amber-50 border border-amber-200 rounded">
                <div className="text-[10px] text-amber-700 mb-1">Stop Sale</div>
                <RategenMarkdown content={hotelDetails?.stop_sale || "N/A"} className="text-xs text-amber-800" />
              </div>
            </div>
          </section>
        )}
        ...
        <section className="space-y-1">
          <h3 className="text-xs font-medium text-muted-foreground">Seasonal Rates</h3>
          {hotelDetails?.seasons?.length > 0 ? (
            <table className="w-full text-xs">
              <thead className="bg-muted/30">
                <tr>
                  <th className="p-1 text-left font-medium">Season</th>
                  <th className="p-1 text-right font-medium">Rate/Night</th>
                  <th className="p-1 text-right font-medium">Single PP</th>
                  <th className="p-1 text-right font-medium">Double PP</th>
                  <th className="p-1 text-right font-medium">Extra Bed</th>
                  <th className="p-1 text-right font-medium">Child (No Bed)</th>
                </tr>
              </thead>
              <tbody>
                {hotelDetails?.seasons?.map((season: any, idx: number) => (
                  <tr key={idx} className="border-b">
                    <td className="p-1">{season.dates || "All Season"}</td>
                    <td className="p-1 text-right">{season.rate_per_night || 0}</td>
                    <td className="p-1 text-right">{season.single_pp || 0}</td>
                    <td className="p-1 text-right">{season.double_pp || 0}</td>
                    <td className="p-1 text-right">{season.extra_bed_pp || 0}</td>
                    <td className="p-1 text-right">{season.child_no_bed || 0}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ) : (
            <p className="text-xs text-muted-foreground">No seasonal rates</p>
          )}
        </section>
