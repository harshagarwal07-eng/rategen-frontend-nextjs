...
export function TourPricingTab({ ctx }: TourPricingTabProps) {
  const {
    formData,
    tourDetails,
    breakups,
  ...
  const checkInDate = itineraryInfo?.checkIn || formData.start_date || formData.day_date;
  ...
  const details = tourDetails || formData;
  const seasons = details?.seasons || [];

  return (
    <div className="flex flex-col h-full">
      <div className="flex-1 space-y-4 p-3">
        {/* Seasonal Rates from Tour Details */}
        {seasons.length > 0 && (
              ...
              <table className="w-full text-xs">
                <thead className="bg-muted/30">
                  <tr>
                    ...
                    <th className="p-2 text-center font-medium">PVT</th>
                  </tr>
                  <tr className="bg-muted/10">
                    ...
                    <th className="p-1 text-center text-[10px] text-muted-foreground">Infant</th>
                    <th className="p-1 text-center text-[10px] text-muted-foreground">Teen</th>
                    <th className="p-1 text-center text-[10px] text-muted-foreground">Adult</th>
                    <th className="p-1 text-center text-[10px] text-muted-foreground">Child</th>
                    <th className="p-1 text-center text-[10px] text-muted-foreground">Per Vehicle</th>
                  </tr>
                </thead>
                <tbody>
                  {seasons.map((season: any, idx: number) => (
                    <tr key={idx} className="border-t">
                      ...
                      <td className="p-2 text-center">{season.ticket_only_rate_adult || "-"}</td>
                      <td className="p-2 text-center">{season.ticket_only_rate_child || "-"}</td>
                      <td className="p-2 text-center">{season.ticket_only_rate_infant || "-"}</td>
                      <td className="p-2 text-center">{season.ticket_only_rate_teenager || "-"}</td>
                      <td className="p-2 text-center">{season.sic_rate_adult || "-"}</td>
                      <td className="p-2 text-center">{season.sic_rate_child || "-"}</td>
                      <td className="p-2 text-center">
                        {season.pvt_rate && Object.keys(season.pvt_rate).length > 0 ? (
                          <div className="flex flex-col gap-1">
                            {Object.entries(season.pvt_rate).map(([vehicle, rate]) => (
                              <span key={vehicle} className="text-[10px]">
                                {vehicle}: {rate as number}
                              </span>
                            ))}
                          </div>
                        ) : (
