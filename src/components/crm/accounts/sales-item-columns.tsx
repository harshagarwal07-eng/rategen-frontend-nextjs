...
import { SalesByItem } from "@/types/ops-accounts";
...
export function generateSalesItemColumns(): ColumnDef<SalesByItem>[] {
  return [
    {
      id: "service_name",
    ...
    },
    {
      ...
      header: ({ column }) => (
        <DataTableColumnFilter column={column} title="Sale Count" enableFiltering={false} />
      ),
      enableSorting: true,
      cell: ({ row }) => (
        <div className="text-right text-sm tabular-nums">
          <span className="font-medium">{Number(row.original.sale_count)}</span>{" "}
          <span className="text-xs text-muted-foreground">{row.original.sale_unit}</span>
        </div>
      ),
      size: 140,
    },
