...
import type { SalesByAgent, SalesByItem } from "@/types/ops-accounts";
import { SalesAgentTab } from "./sales-agent-tab";
import { SalesItemTab } from "./sales-item-tab";
import { Button } from "@/components/ui/button";
import { Skeleton } from "@/components/ui/skeleton";
import { cn } from "@/lib/utils";
import { Funnel } from "lucide-react";

type SalesView = "agent" | "item";

const VIEWS: { value: SalesView; label: string }[] = [
  { value: "agent", label: "Sales By Agent" },
  { value: "item", label: "Sales By Service" },
];

interface SalesClientWrapperProps {
  initialView: SalesView;
  initialAgentData: { data: SalesByAgent[]; totalItems: number };
  initialItemData: { data: SalesByItem[]; totalItems: number };
}
...
export function SalesClientWrapper({
  ...
  const [agentData, setAgentData] = React.useState(initialAgentData);
  ...
  const view = (params.view === "item" ? "item" : "agent") as SalesView;

  const handleViewChange = (nextView: SalesView) => {
    ...
    startTransition(async () => {
      ...
      if (nextView === "agent") {
        const data = await getSalesByAgent(commonParams);
        setAgentData(data);
      } else {
        const data = await getSalesByItem(commonParams);
        setItemData(data);
      }
  ...
  const title = view === "item" ? "Sales By Service" : "Sales By Agent";
    ...
    <>
      ...
      <div className="flex-1 px-4 py-4 flex flex-col overflow-hidden gap-3">
        {isPending ? (
          ...
          <SalesAgentTab data={agentData} />
        ) : (
          <SalesItemTab data={itemData} />
        )}
      </div>
    </>
  );
