...
import { SalesByAgent } from "@/types/ops-accounts";
...
export function generateSalesAgentColumns(): ColumnDef<SalesByAgent>[] {
  return [
    {
      id: "agent",
      accessorKey: "agency_name",
      header: ({ column }) => <DataTableColumnFilter column={column} title="Agent" enableFiltering={false} />,
      enableSorting: true,
      cell: ({ row }) => <div className="text-sm font-medium">{row.original.agency_name}</div>,
      size: 200,
    },
    {
      id: "service_types",
      ...
      header: ({ column, table }) => {
        ...
        return (
          <DataTableColumnFilter column={filterCol ?? column} title="Service Type" options={SERVICE_TYPE_OPTIONS} />
        );
      },
      enableSorting: false,
      cell: ({ row }) => {
    ...
    },
    {
      ...
      enableSorting: true,
      cell: ({ row }) => <div className="text-right text-sm tabular-nums">{row.original.service_count}</div>,
      size: 80,
    },
    {
      id: "pax",
