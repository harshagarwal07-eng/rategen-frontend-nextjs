...
import { getItemSalesByAgent } from "@/data-access/ops-accounts";
import type { SalesByItem, ItemSalesByAgent } from "@/types/ops-accounts";

interface ItemAgentsSheetProps {
  item: SalesByItem | null;
  onClose: () => void;
  startDate?: string;
  endDate?: string;
}

export function ItemAgentsSheet({ item, onClose, startDate, endDate }: ItemAgentsSheetProps) {
  const [rows, setRows] = React.useState<ItemSalesByAgent[]>([]);
  ...
  React.useEffect(() => {
    ...
    let cancelled = false;
    setLoading(true);
    getItemSalesByAgent(item.item_id, startDate, endDate)
      .then((data) => {
  ...
  const totalSaleCount = rows.reduce((s, r) => s + Number(r.sale_count), 0);
      ...
      <SheetContent side="right" className="w-full sm:max-w-2xl p-0 flex flex-col">
        <SheetHeader className="px-5 shrink-0 space-y-3">
          <div>
            ...
            <SheetDescription className="text-xs mt-0.5">Breakdown by agent</SheetDescription>
          </div>
            ...
            <div className="inline-flex items-center gap-4 px-3 py-2 rounded-md bg-card divide-x [&>div]:pr-2 shrink-0">
              <div>
                ...
                <p className="text-sm font-semibold font-mono tabular-nums">
                  {totalSaleCount}{" "}
                  <span className="text-xs font-normal text-muted-foreground">{item.sale_unit}</span>
                </p>
              </div>
              <div>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-muted/30">
                    <TableHead className="min-w-[160px] text-xs">Agent</TableHead>
                    <TableHead className="w-24 text-xs">Type</TableHead>
                    <TableHead className="w-32 text-right text-xs">Sales</TableHead>
                    ...
                    <>
                      {Array.from({ length: 3 }).map((_, i) => (
                        <TableRow key={i} className="hover:bg-muted/30">
                          <TableCell><Skeleton className="h-4 w-11/12" /></TableCell>
                    ...
                    <>
                      {rows.map((row, idx) => {
                        ...
                        return (
                          <TableRow key={idx} className="hover:bg-muted/30">
                            <TableCell>
                              <p className="text-xs font-medium leading-tight">{row.agency_name || "—"}</p>
                            </TableCell>
                            <TableCell>
                              <Badge
                                variant="secondary"
                            ...
                            <TableCell className="text-right tabular-nums text-xs">
                              <span className="font-medium">{Number(row.sale_count)}</span>{" "}
                              <span className="text-muted-foreground">{row.sale_unit}</span>
                            </TableCell>
                            <TableCell className="text-right tabular-nums text-xs">
                      ...
                      <TableRow className="bg-muted/30 border-t-2">
                        ...
                        <TableCell className="text-right tabular-nums text-xs font-semibold">
                          {totalSaleCount}{" "}
                          <span className="font-normal text-muted-foreground">{item.sale_unit}</span>
                        </TableCell>
                        <TableCell />
                        <TableCell className="text-right tabular-nums text-xs font-semibold">
