...
import { useClipboard } from "@/components/excel/use-clipboard";
import { useHistory } from "@/components/excel/use-history";
import { CellEditor } from "@/components/excel/cell-editors";
import { ColumnFilter } from "@/components/excel/column-filter";
import { DateRangeEditor } from "@/components/excel/date-range-editor";
import { ExpandableEditor } from "@/components/excel/expandable-editor";
import { DurationEditor } from "@/components/excel/duration-editor";
import { MultiSelectEditor } from "@/components/excel/multi-select-editor";
...
interface ExcelGridProps<T extends { id: string }> {
  ...
  /** Error message shown in the toolbar beside the search (e.g. duplicate vehicle warning) */
  gridError?: string;
  ...
  // When this value changes, scroll the grid horizontally to the end (e.g. after column count change)
  scrollToEndTrigger?: unknown;
}
...
export function ExcelGrid<T extends { id: string }>({
  ...
  // When scrollToEndTrigger changes (e.g. column count), scroll horizontally to the end
  useEffect(() => {
    if (scrollToEndTrigger === undefined) return;
  ...
  const handleCopyButtonClick = useCallback(
    (rowIdx: number) => {
      ...
      // Use duplicate if available (for transfers), otherwise insert
      if (onDuplicateRow) {
  ...
  const renderCell = (rowIdx: number, colIdx: number, value: any, cellWidth?: number) => {
    ...
    const row = filteredData[rowIdx];
    const isTransferLocked = getIsLocked(row, column.accessorKey as string, "transfer");
    const isPackageLocked = getIsLocked(row, column.accessorKey as string, "transfer_package");
      ...
      return (
        ...
        >
          <Autocomplete
            ...
            value={value ?? ""}
            onChange={(newValue) => {
              if (column.editable !== false && !isTransferLocked && !isPackageLocked) {
                handleCellChangeWithHistory(getRowId(row), column.accessorKey, newValue);
              }
              containerRef.current?.focus();
            }}
            disabled={column.editable === false || isTransferLocked || isPackageLocked}
            placeholder="--"
            searchPlaceholder="Search..."
          />
        </div>
    ...
    // Multi-select type - same pattern as Autocomplete: single-line trigger + popover (contained in cell)
        ...
        >
          <MultiSelectEditor
            ...
            onValuesChange={(newValues) => {
              if (column.editable !== false && !isTransferLocked && !isPackageLocked) {
                handleCellChangeWithHistory(getRowId(row), column.accessorKey, newValues);
              }
              containerRef.current?.focus();
            }}
            options={column.options as { group: string; options: IOption[] }[]}
            disabled={column.editable === false || isTransferLocked || isPackageLocked}
            onClose={() => containerRef.current?.focus()}
          />
        </div>
      );
    }
    ...
    // Duration type - show formatted duration, edit with popover
    if (column.type === "duration") {
      if (isEditing) {
        return (
          <DurationEditor
            value={value as { days?: number; hours?: number; minutes?: number } | null | undefined}
            onSave={(newValue) => {
              if (column.editable !== false && !isTransferLocked && !isPackageLocked) {
                handleCellChangeWithHistory(getRowId(row), column.accessorKey, newValue);
              }
      ...
      // Display formatted duration when not editing - pad hours and minutes with leading zeros
      const duration = value as { days?: number; hours?: number; minutes?: number } | null | undefined;
      const displayValue = duration
        ? [
            duration.days !== undefined && duration.days !== null ? `${duration.days}d` : "",
        ...
        : "";
      return (
        <div className="cell-content" title={displayValue || undefined}>
          {displayValue}
        </div>
      );
    }
    ...
    if (colIdx === 0 && (onViewRow || isTransferLocked)) {
      return (
        <div className="cell-content cell-with-action" title={textValue}>
          <span className="cell-text">{textValue}</span>
          {value && isTransferLocked && (
            <IndicateLocked tooltip="This transfer is linked to datastore" className="size-3!" />
          )}
          {value && onViewRow && (
            <button
    ...
    // for package name column
    if (column.id === "name" && isPackageLocked) {
      return (
        <div className="cell-content" title={textValue}>
          <span className="cell-text">{textValue}</span>
          {value && isPackageLocked && (
            <IndicateLocked tooltip="This package is linked to datastore" className="size-3!" />
          )}
        </div>
      );
    }
      ...
      {showToolbar && (
        <div className="excel-grid-toolbar">
          ...
          <div className="excel-grid-toolbar-right">
            {gridError && (
              <span className="text-xs text-destructive ml-2" title={gridError}>
                {gridError}
              </span>
            )}
