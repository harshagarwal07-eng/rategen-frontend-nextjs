...
import { AgePolicyBand } from "@/types/transfers";

const PRESET_NAMES = ["Adult", "Child", "Infant", "Teenager", "Senior", "Youth"];

export type AgeBandRow = AgePolicyBand & { _key: string; _useCustomName: boolean };

export function bandsToRows(bands: AgePolicyBand[]): AgeBandRow[] {
...
export function rowsToBands(rows: AgeBandRow[]): AgePolicyBand[] {
  return rows.map((r, i) => ({
...
export default function AgePolicySection({ rows, onChange }: AgePolicySectionProps) {
  ...
  function handlePreset(key: string, value: string) {
    if (value === "__custom__") updateRow(key, { band_name: "", _useCustomName: true });
    ...
    <div>
      ...
      <Table>
        ...
        <TableBody>
          ...
          {rows.map((row, idx) => (
            <TableRow key={row._key} className={idx % 2 === 1 ? "bg-muted/30" : ""}>
              <TableCell className="py-1.5 pr-2">
                {row._useCustomName ? (
                  <Input
                    autoFocus
                    value={row.band_name}
                    onChange={(e) => updateRow(row._key, { band_name: e.target.value })}
                ...
                {errors[row._key] && (
                  <p className="mt-0.5 text-[10px] text-red-500">{errors[row._key]}</p>
                )}
              </TableCell>
