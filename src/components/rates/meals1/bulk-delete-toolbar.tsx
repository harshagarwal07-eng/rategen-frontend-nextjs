...
export function BulkDeleteToolbar({ selectedRows, onClearSelection }: BulkDeleteToolbarProps) {
  ...
  const handleBulkDelete = async () => {
    try {
      setLoading(true);
      const results = await Promise.all(selectedRows.map((row) => deleteMeal1(row.id!)));
      const errors = results.filter((r) => r.error);
      if (errors.length > 0) throw new Error(`Failed to delete ${errors.length} meal(s)`);
      toast.success(`Deleted ${selectedRows.length} meal(s)`);
      onClearSelection();
      queryClient.invalidateQueries({ queryKey: ["meals1"], exact: false, type: "active" });
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Failed to delete meals");
    } finally {
      setLoading(false);
      ...
      <Dialog open={showConfirmDialog} onOpenChange={setShowConfirmDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Delete {selectedRows.length} meal{selectedRows.length !== 1 ? "s" : ""}? This action cannot be undone.
            </DialogDescription>
          </DialogHeader>
