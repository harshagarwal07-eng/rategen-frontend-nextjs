...
// Taxes editor for tour packages.
// Forked from transfers' taxes-editor because it imports `PackageTax`
// from `@/types/transfers`. Tour packages use `TourPackageTax` (same
...
import { TourPackageTax } from "@/types/tours";
...
export function taxesToRows(taxes: TourPackageTax[]): TaxRow[] {
...
export function rowsToTaxes(rows: TaxRow[]): TourPackageTax[] {
  return rows.map((r) => ({
    name: r.name.trim(),
...
export default function TaxesEditor({ rows, onChange }: TaxesEditorProps) {
  function updateRow<K extends keyof TaxRow>(
    key: string,
    field: K,
    value: TaxRow[K],
  ) {
                ...
                <div className="grid grid-cols-[1fr_100px_140px_160px_32px] gap-x-3 items-center">
                  ...
                  <div className="flex items-center gap-2">
                    <Switch
                      ...
                      onCheckedChange={(v) =>
                        updateRow(row._key, "is_inclusive", v)
                      }
                    />
