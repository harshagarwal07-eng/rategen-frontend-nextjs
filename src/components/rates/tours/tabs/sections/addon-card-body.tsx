...
// per-band rates, total-rate tiers, package links, images) is hoisted
// into the parent so the parent's save() handle can flush in one shot.
// Image add/delete self-persists (mirrors Tab 1's image section).
...
interface AddonCardBodyProps {
  ...
  sortedBands: TourAddonAgePolicyBand[];
  ...
  images: TourAddonImage[];
  setImages: (next: TourAddonImage[]) => void;
}
...
export default function AddonCardBody({
  ...
  async function handleAddImage() {
    ...
    setAddingImage(true);
    const res = await addAddonImage(addonId, { url });
  ...
  async function handleDeleteImage(imageId: string) {
    const res = await deleteAddonImage(imageId);
    if (res.error) {
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="flex items-center justify-between border rounded-md px-3 h-10">
                <FormLabel className="mt-0 text-xs">Mandatory</FormLabel>
                <FormControl>
                  <Switch
