...
const MultiSelectorTrigger = forwardRef<HTMLDivElement, MultiSelectorTriggerProps>(
  ({ className, children, data, keyString = "label", valueString = "value", ...props }, ref) => {
    ...
    return (
      <div
        ref={ref}
        className={cn(
          "flex flex-wrap gap-1 p-1 py-1.5 focus-visible:outline-none focus-visible:border-ring focus-visible:ring-ring/50 aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 focus-visible:ring-[3px] px-3 hover:bg-input/40 text-foreground rounded-md min-h-10 hover:text-muted-foreground bg-transparent dark:bg-input/30 dark:hover:bg-input/50 border-2 font-normal",
          className
        )}
        {...props}
      >
        <TooltipProvider delayDuration={300}>
          {_value.map((item, index) => {
            const itemLabel = (data.find((d) => (d as any)[valueString] === item) as any)?.[keyString] || item;
            return (
              <Tooltip key={item}>
                <TooltipTrigger asChild>
                  <Badge
                    className={cn(
                      "px-2 rounded-xl flex items-center gap-1 max-w-[150px]",
                      activeIndex === index && "ring-2 ring-muted-foreground "
                    )}
                    variant={"outline"}
                  >
                    <span className="text-xs truncate block">{itemLabel}</span>
                    <button
                    ...
                    >
                      ...
                      <RemoveIcon className="w-4 h-4 hover:stroke-destructive shrink-0" />
                    </button>
                  </Badge>
                </TooltipTrigger>
                <TooltipContent>
                  <p className="max-w-xs">{itemLabel}</p>
                </TooltipContent>
              </Tooltip>
            );
          })}
...
>(({ className, ...props }, ref) => {
  ...
  return (
    <CommandPrimitive.Input
      ...
      className={cn(
        "ml-2 bg-transparent outline-none placeholder:text-muted-foreground placeholder:text-sm flex-1 disabled:cursor-not-allowed disabled:opacity-50",
        className,
...
>(({ className, children }, ref) => {
  return (
    <CommandList
      ref={ref}
      className={cn(
        "p-2 flex flex-col gap-2 rounded-md scrollbar-thin scrollbar-track-transparent transition-colors scrollbar-thumb-muted-foreground dark:scrollbar-thumb-muted scrollbar-thumb-rounded-lg w-full absolute bg-popover shadow-md z-20 border top-0",
        className
      )}
    >
      {children}
