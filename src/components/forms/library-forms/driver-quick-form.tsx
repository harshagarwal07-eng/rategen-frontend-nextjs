...
const driverQuickSchema = z.object({
...
type DriverQuickData = z.infer<typeof driverQuickSchema>;

export function DriverQuickForm({
  supplierId,
  onSave,
}: {
  supplierId?: string;
  ...
  const form = useForm<DriverQuickData>({
    resolver: zodResolver(driverQuickSchema),
    defaultValues: {
      name: "",
      gender: "",
      phone: "",
  ...
  const onSubmit = async (data: DriverQuickData) => {
    setIsLoading(true);
    try {
      const result = await addLibraryItem("drivers", {
        ...data,
