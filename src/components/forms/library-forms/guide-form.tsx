...
import { GuideLibraryFormData, guideLibraryFormSchema } from "../schemas/library-schema";
...
interface GuideFormProps {
  initialData: Partial<GuideLibraryFormData>;
...
interface GuideFullscreenFormProps {
  isOpen: boolean;
  onClose: () => void;
  initialData?: (GuideLibraryFormData & { id?: string }) | null;
  onSuccess?: () => void;
}

function GuideForm({ initialData, onNext, formRef }: GuideFormProps) {
  const { user } = useUser();
  ...
  const form = useForm<GuideLibraryFormData>({
    resolver: zodResolver(guideLibraryFormSchema),
    defaultValues: {
  ...
  const onSubmit = async (data: GuideLibraryFormData) => {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-2xl font-bold mb-2">{initialData?.name ? "Edit Guide" : "Create Guide"}</h2>
        <p className="text-muted-foreground">Enter the guide details and payroll information</p>
      </div>
                    ...
                    <FormItem>
                      <FormLabel className="flex justify-between items-center">
                        ...
                        <div className="flex items-center gap-2 mr-4">
                          ...
                          />
                          <label htmlFor="guide-same-phone" className="text-xs text-muted-foreground cursor-pointer">
                            Same as phone
                          </label>
                        </div>
                      </FormLabel>
...
export default function GuideFullscreenForm({ isOpen, onClose, initialData, onSuccess }: GuideFullscreenFormProps) {
  const router = useRouter();
  const [formData, setFormData] = useState<Partial<GuideLibraryFormData>>({});
    ...
    async (data: any) => {
      ...
      try {
        ...
        if ((initialData as any)?.id) {
          ...
          const result = await updateLibraryItem("guides", (initialData as any).id, updatedFormData as any);
          if ("error" in result && result.error) throw new Error(result.error);
          toast.success("Guide updated successfully!");
        } else {
          // Create new guide
          const result = await addLibraryItem("guides", updatedFormData as any);
          if ("error" in result && result.error) throw new Error(result.error);
        ...
        toast.error("Failed to save guide data");
      } finally {
        setIsLoading(false);
      }
    },
            ...
            <div className="flex justify-center">
              <div className="flex bg-muted rounded-lg p-1">
                ...
                >
                  ...
                  <span>{(initialData as any)?.id ? "Edit Guide" : "Create Guide"}</span>
                </div>
              </div>
            </div>
          </DialogHeader>
