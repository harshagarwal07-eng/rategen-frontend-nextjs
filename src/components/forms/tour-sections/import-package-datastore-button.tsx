...
import { ITourAddOn, ITourPackage } from "../schemas/tours-datastore-schema";
import { getTourDatastoreById } from "@/data-access/tours-datastore";
import { EyePopover } from "@/components/common/eye-popover";
import { Tour } from "@/types/tours";

type Props = {
  tourDatastoreId: string;
  currPackages: ITourPackage[];
  onImport?: (packages: ITourPackage[], addOns: ITourAddOn[]) => Promise<void>;
};

export default function ImportPackagesButton({ tourDatastoreId, currPackages, onImport }: Props) {
  const [open, setOpen] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [packagesDatastore, setPackagesDatastore] = useState<ITourPackage[]>([]);
  const [addOnsDatastore, setAddOnsDatastore] = useState<ITourAddOn[]>([]);
  ...
  const existingPackageDatastoreIds = useMemo(() => {
    return new Set(
      currPackages
        .filter((pkg) => (pkg as any).tour_package_datastore_id && !(pkg as any).is_unlinked)
        .map((pkg) => (pkg as any).tour_package_datastore_id)
    );
  }, [currPackages]);
  ...
  const fetchPackages = useCallback(async () => {
    try {
      setIsLoading(true);
      const { data: tour, error } = await getTourDatastoreById(tourDatastoreId);
      ...
      setPackagesDatastore(tour.packages || []);
      setAddOnsDatastore(tour.add_ons || []);
    } catch (error) {
      toast.error("Failed to load packages from datastore");
      console.error(error);
    } finally {
  ...
  const columns: ColumnDef<ITourPackage>[] = useMemo(
    () => [
      {
        id: "select",
        size: 50,
      ...
      },
      {
        ...
        accessorKey: "name",
        header: "Package Name",
        cell: ({ cell }) => <p className="font-medium text-sm capitalize">{cell.getValue<ITourPackage["name"]>()}</p>,
        enablePinning: true,
      },
      {
        id: "description",
        ...
        header: "Description",
        cell: ({ cell }) => {
          return <EyePopover title="Description" description={cell.getValue<ITourPackage["description"]>() || "-"} />;
        },
      },
      {
        id: "duration",
      ...
      },
      {
        ...
        header: "Inclusions",
        cell: ({ cell }) => {
          let value = cell.getValue<ITourPackage["inclusions"]>();
      ...
      },
      {
        ...
        header: "Exclusions",
        cell: ({ cell }) => {
          let value = cell.getValue<ITourPackage["exclusions"]>();
      ...
      },
      {
        ...
        header: "Preferred",
        cell: ({ cell }) => <div>{cell.getValue<ITourPackage["preferred"]>() ? "Yes" : "No"}</div>,
      },
      {
        id: "includes_transfer",
        accessorKey: "includes_transfer",
        header: "Transfer Included",
        cell: ({ cell }) => <div>{cell.getValue<ITourPackage["includes_transfer"]>() ? "Yes" : "No"}</div>,
      },
      {
        id: "addon_count",
  ...
  const { table } = useDataTable<ITourPackage>({
