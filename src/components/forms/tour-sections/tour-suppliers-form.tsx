...
interface TourSuppliersFormProps {
  initialData?: {
    id?: string;
...
export default function TourSuppliersForm({ initialData, onNext, formRef }: TourSuppliersFormProps) {
  ...
  const [isLoadingInitial, setIsLoadingInitial] = useState(!!tourId);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!tourId) {
      toast.error("Please save Tour Info first before managing suppliers.");
      return;
    }
  ...
  if (!tourId) {
    return (
      <div className="flex items-center justify-center h-60">
        <p className="text-sm text-muted-foreground">Save Tour Info first to enable supplier linking.</p>
      </div>
    );
  }
                        ...
                        <div className="space-y-1.5">
                          ...
                          {packageOptions.length === 0 ? (
                            ...
                            <MultiSelector
                              ...
                              onValuesChange={(ids) =>
                                setCards((prev) => prev.map((c, i) => (i === index ? { ...c, package_ids: ids } : c)))
                              }
                            >
