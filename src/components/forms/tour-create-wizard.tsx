...
import TourGeneralInfoForm, {
  TourGeneralInfoValues,
} from "@/components/rates/tours/tabs/tab1-general-info";
import Tab2Packages from "@/components/rates/tours/tabs/tab2-packages";
import Tab3SeasonsRates from "@/components/rates/tours/tabs/tab3-seasons-rates";
import Tab4Addons from "@/components/rates/tours/tabs/tab4-addons";
import {
  createTour,
  updateTour,
  getTourById,
...
interface TourCreateWizardProps {
  isOpen: boolean;
  onClose: () => void;
  initialData?: Partial<TourListRow> | null;
...
type FormState = Partial<TourDetail>;

export default function TourCreateWizard({
  isOpen,
  onClose,
  initialData,
  countries,
    ...
    (async () => {
      ...
      if (full.data) {
        ...
        setContextInfo({
          name: full.data.name ?? "",
          countryName:
            countries.find((c) => c.id === full.data!.country_id)
              ?.country_name ?? "",
        });
      }
    })();
  ...
  const handleNext = useCallback(
    async (data: TourGeneralInfoValues) => {
      setIsLoading(true);
      try {
        const fullPayload = {
          name: data.name,
        ...
        let savedId: string;
        if (formData.id) {
          const result = await updateTour(formData.id, fullPayload);
        ...
        if (full.data) {
          ...
          setFormData((prev) => ({ ...prev, ...fullPayload, id: savedId }));
        }

        setTab1Dirty(false);
        toast.success(formData.id ? "Saved." : "Tour created.");
      ...
      <Dialog open={isOpen} onOpenChange={(open) => !open && handleClose()}>
        ...
        >
          ...
          <DialogDescription className="sr-only">
            {formData.id ? "Edit tour" : "Create a tour product"}
          </DialogDescription>
            ...
            <div className="border-b bg-background px-6 py-3 flex items-center justify-between">
              <div className="flex items-center gap-2 text-sm">
                {displayName ? (
                  ...
                  <span className="text-muted-foreground italic">New Tour</span>
                )}
                {displayCountry && (
                  <>
                ...
                <div className="flex items-center gap-3">
                  ...
                  {currentStep === 2 && (
                    <Button
                      ...
                      className="min-w-32"
                      title={
                        tab3Dirty ? "Save package changes first" : undefined
                      }
                    >
                      Save & Continue
                    </Button>
                  ...
                  {currentStep === 3 && (
                    <Button
                      onClick={handleSaveClick}
                      className="min-w-32"
                      disabled={isLoading || anyDirty}
                      title={
                        anyDirty
                          ? "Resolve unsaved changes in earlier tabs first"
                          : undefined
                      }
                    >
                      {isLoading ? (
