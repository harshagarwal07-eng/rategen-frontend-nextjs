...
interface GuideSuppliersFormProps {
...
export default function GuideSuppliersForm({ initialData, onNext, formRef }: GuideSuppliersFormProps) {
  ...
  const [isLoadingInitial, setIsLoadingInitial] = useState(!!guideId);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!guideId) {
      toast.error("Please save Guide Info first before managing suppliers.");
      return;
    }
  ...
  if (!guideId) {
    return (
      <div className="flex items-center justify-center h-60">
        <p className="text-sm text-muted-foreground">Save Guide Info first to enable supplier linking.</p>
      </div>
    );
  }
