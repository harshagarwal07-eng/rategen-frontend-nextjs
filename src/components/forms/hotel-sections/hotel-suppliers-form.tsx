...
interface HotelSuppliersFormProps {
...
export default function HotelSuppliersForm({ initialData, onNext, formRef }: HotelSuppliersFormProps) {
  ...
  const [isLoadingInitial, setIsLoadingInitial] = useState(!!hotelId);
  ...
  const loadSuppliers = async (inactiveLinked?: SupplierAssociation[]) => {
    ...
    // Inject inactive linked suppliers so their name resolves in the selector
  ...
  // Seed cards from initialData — no extra fetch needed, data already loaded by parent
  useEffect(() => {
    if (!hotelId) return;
    const assocs = initialData?.supplier_associations ?? [];
    if (assocs.length === 0) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!hotelId) {
      toast.error("Please save General Info first before managing suppliers.");
      return;
    }
  ...
  // IDs already picked across all cards (excluding the card's own so it can re-pick itself)
  ...
  if (!hotelId) {
    return (
      <div className="flex items-center justify-center h-60">
        <p className="text-sm text-muted-foreground">Save General Info first to enable supplier linking.</p>
      </div>
    );
  }
              ...
              <div key={index} className="relative">
                <BorderedCard title={cards.length > 1 ? `Supplier ${index + 1}` : "Supplier"}>
                  <div className="space-y-4">
                    ...
                    {/* POCs — only shown once a supplier is selected */}
                    {card.supplier_id && (
