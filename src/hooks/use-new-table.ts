...
interface UseNewTableProps<TData>
  extends Omit<
    TableOptions<TData>,
...
export function useNewTable<TData>(props: UseNewTableProps<TData>) {
  const {
    columns,
    pageCount = -1,
    initialState,
  ...
  const onColumnSizingChange = React.useCallback(
    (updaterOrValue: Updater<ColumnSizingState>) => {
      setColumnSizing((prev) => {
        ...
        for (const [colId, size] of Object.entries(next)) {
          const colDef = columns.find((c) => c.id === colId);
          const minSize = colDef?.size ?? 0;
          clamped[colId] = Math.max(size as number, minSize);
        }
        return clamped;
      });
    },
  ...
  const [perPage, setPerPage] = useQueryState(
    PER_PAGE_KEY,
    parseAsInteger.withOptions(queryStateOptions).withDefault(initialState?.pagination?.pageSize ?? 100)
  );
